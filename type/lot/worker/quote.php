...
if (strpos($content, '</p>') !== false) {
    if (strpos($content, '</blockquote>') !== false) {
        if (strpos($content, '<blockquote ') === false) {
            return str_replace('<blockquote>', '<blockquote class="quote">', $content);
        }
        return preg_replace_callback('#<blockquote(?:\s[^<>]*?)?>#', function($m) {
            $quote = HTML::apart($m[0]);
            if (isset($quote[2]['class'])) {
                $quote[2]['class[]'] = array_merge(['quote'], explode(' ', $quote[2]['class']));
                unset($quote[2]['class']);
            } else {
                $quote[2]['class[]'] = ['quote'];
            }
